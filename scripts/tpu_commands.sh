...
function _tpu_setup {
    ...
    for host in $tpu_ips; do
        ssh $host '~/ado/scripts/tpu_vm_setup.sh' &
    done
    wait &> /dev/null
}
...
function _tpu_copy {
    ...
    tpu_ips=$(_tpu_ips $tpu_zone $tpu_project $tpu_name)
    for host in $tpu_ips; do
        rsync -e "ssh -o StrictHostKeyChecking=no" -avPI --exclude=logs --exclude=__pycache__ --exclude=.git --exclude='*_env' --exclude='*ipynb' --exclude='*npy' --exclude=data --exclude=notebook --exclude=wandb $PROJECT_HOME/$PROJECT_NAME $host:~/ &
    done
    wait &> /dev/null
    sleep 1
}
...
function tpu {
    ...
    export PROJECT_HOME='..'
    export PROJECT_NAME='ado'
    if [ "$1" = "ado_project" ]; then
        tpu_project='YOUR_PROJECT_NAME'  # TODO: replace with your GCP project id.
        tpu_zone='europe-west4-a'  # TODO: update if your TPUs are in a different region.
        tpu_gen='v3'  # TODO: update if your TPUs are v4 or later.
    else
        echo "Invalid syntax!"
        trap - SIGINT SIGTERM
        return 1
    ...
    elif [ "$2" = "reset_key" ]; then
        _tpu_reset_key $tpu_zone $tpu_project $3
    else
        echo "Invalid syntax!"
        trap - SIGINT SIGTERM
        return 1
